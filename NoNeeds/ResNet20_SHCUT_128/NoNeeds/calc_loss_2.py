...
def normalize_direction(direction, weights):
    ...
    for d, w in zip(direction, weights):
        norm_dir.append(d*(LA.norm(w) / (LA.norm(d) + 1e-10)))
    return np.array(norm_dir)
...
def overwrite_weights(model, init_weights, directions, step):
    ...
    for (p, w, d) in zip(model.get_weights(), init_weights, changes):
        p = np.array(w) + np.array(d)

        #data.append(p)
        return [p for p in model.get_weights()]

def overwrite_weights_2(model,step):
    weights=model.get_weights()
    dir_x=get_random_weights(weights)
    dx=[]
    for (d,w) in zip(dir_x,weights):
        dx.append(normalize_direction(d, w))
        # if d.ndim == 1:
        #     d.fill(0)
        #     dx.append(d)
        # else:
        #     dx.append(normalize_direction(d, w))
    dir_y = get_random_weights(weights)
    dy=[]
    for (d,w) in zip(dir_y,weights):
        dy.append(normalize_direction(d, w))
        # if d.ndim==1:
        #     d.fill(0)
        #     dy.append(d)
        # else:
    ...
    new_weights=[]
    for (w,c) in zip(weights,changes):
        n_w=w+c
