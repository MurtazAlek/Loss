...
def calulate_loss_landscape(model, directions):
    ...
    #print('in bef',init_weights[0][0][0][0])
    ...
    with h5py.File("./3d_surface_file_ResNet20_SHCUT_128.h5", 'r+') as f:
        ...
        for count, ind in enumerate(inds):
            ...
            #print('bef',model.get_weights()[0][0][0][0])
            #overwrite_weights(model, init_weights, directions, coord)
            model.set_weights(overwrite_weights(model, init_weights, directions, coord))
            model.compile(loss=loss_fn,optimizer=opt,metrics=metrica)
            #print('af', model.get_weights()[0][0][0][0])
...
def setup_surface_file():
    xmin, xmax, xnum = -0.8, 1, 10
    ymin, ymax, ynum = -0.8, 1, 10
...
def overwrite_weights(model, init_weights, directions, step):
    dx = directions[0]
    dy = directions[1]
    for (d0, d1) in zip(dx, dy):
        print('d0',d0)
        print('d1',d1)
        print('step',step)
        changes = d0 * step[0] + d1 * step[1]
        print('c', changes)
...
# import pandas as pd
# filename = "3d_surface_file_ResNet20_NOSHCUT_128.h5"
#
